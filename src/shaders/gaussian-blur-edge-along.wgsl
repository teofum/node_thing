...
fn sample_tangent(coord: vec2f) -> vec3f {
  let int_coord = vec2u(coord);
  let frac_coord = fract(coord);

  let i00 = raw_tangent[int_coord.x + int_coord.y * u.width];
  let i01 = raw_tangent[int_coord.x + (int_coord.y + 1) * u.width];
  let i10 = raw_tangent[int_coord.x + 1 + int_coord.y * u.width];
  let i11 = raw_tangent[int_coord.x + 1 + (int_coord.y + 1) * u.width];
